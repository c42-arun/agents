from __future__ import annotations
import datetime
from collections.abc import Callable
from dataclasses import dataclass, field
from kiota_abstractions.serialization import Parsable, ParseNode, SerializationWriter
from typing import Any, Optional, TYPE_CHECKING, Union

@dataclass
class LsPartyCreditReport(Parsable):
    # The agreementNumber property
    agreement_number: Optional[str] = None
    # The date property
    date: Optional[datetime.datetime] = None
    # The format property
    format: Optional[str] = None
    # The report property
    report: Optional[str] = None
    # The searchAutoCode property
    search_auto_code: Optional[float] = None
    # The shortName property
    short_name: Optional[str] = None
    # The type property
    type: Optional[str] = None
    
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> LsPartyCreditReport:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: LsPartyCreditReport
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return LsPartyCreditReport()
    
    def get_field_deserializers(self,) -> dict[str, Callable[[ParseNode], None]]:
        """
        The deserialization information for the current model
        Returns: dict[str, Callable[[ParseNode], None]]
        """
        fields: dict[str, Callable[[Any], None]] = {
            "agreementNumber": lambda n : setattr(self, 'agreement_number', n.get_str_value()),
            "date": lambda n : setattr(self, 'date', n.get_datetime_value()),
            "format": lambda n : setattr(self, 'format', n.get_str_value()),
            "report": lambda n : setattr(self, 'report', n.get_str_value()),
            "searchAutoCode": lambda n : setattr(self, 'search_auto_code', n.get_float_value()),
            "shortName": lambda n : setattr(self, 'short_name', n.get_str_value()),
            "type": lambda n : setattr(self, 'type', n.get_str_value()),
        }
        return fields
    
    def serialize(self,writer: SerializationWriter) -> None:
        """
        Serializes information the current object
        param writer: Serialization writer to use to serialize this model
        Returns: None
        """
        if writer is None:
            raise TypeError("writer cannot be null.")
        writer.write_str_value("agreementNumber", self.agreement_number)
        writer.write_datetime_value("date", self.date)
        writer.write_str_value("format", self.format)
        writer.write_str_value("report", self.report)
        writer.write_float_value("searchAutoCode", self.search_auto_code)
        writer.write_str_value("shortName", self.short_name)
        writer.write_str_value("type", self.type)
    

